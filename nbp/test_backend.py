...
<kurs_sredni>4,0989</kurs_sredni>
        ...
        with it.having('a working day'):
            @it.has_setup
            @patch('backend.request')
            @patch('backend.datetime')
            def setup(datetime_mock, request_mock):
                datetime_mock.datetime.now.return_value = datetime.datetime(2017, 1, 28)
                request_mock.urlopen.side_effect = urlopen_mock
                it.datetime_mock = datetime_mock
                it.request_mock = request_mock
            ...
            def test():
                it.request_mock.urlopen.assert_any_call(
                    DIR_TXT_URL)
        ...
        with it.having('a non-working day'):
            ...
            @patch('backend.request')
            @patch('backend.datetime')
            def test(datetime_mock, request_mock):
                datetime_mock.datetime.now.return_value = datetime.datetime(2017, 1, 28)
                request_mock.urlopen.side_effect = urlopen_mock
                it.datetime_mock = datetime_mock
                it.request_mock = request_mock
